...
if(isset($_POST['add_product'])){
   ...
   $units = $_POST['units'];
   $units = filter_var($units, FILTER_SANITIZE_STRING);
   ...
   if($select_products->rowCount() > 0){
      ...
      $insert_products = $conn->prepare("INSERT INTO `products`(name, category, details, price, image,units) VALUES(?,?,?,?,?,?)");
      $insert_products->execute([$name, $category, $details, $price, $image, $units]);
...
<section class="add-products">
   ...
   <form action="" method="POST" enctype="multipart/form-data">
      <div class="flex">
         ...
         <div class="flex">
            <div class="inputBox">
               <select name="units" class="box" required placeholder ="Enter units">
...
<section class="show-products">
   ...
   <div class="box">
      ...
      <?php
         if($fetch_products['units'] != ''){
      ?>
      <div class="name"><?= $fetch_products['name'].' / '.$fetch_products['units']; ?></div>
      <?php
         }else{
      ?>
